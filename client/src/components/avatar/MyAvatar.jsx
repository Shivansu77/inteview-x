...
export function Model({
  ...
  const emotionIndicesRef = useRef([]);
  ...
  useEffect(() => {
    ...
    Object.values(nodes).forEach((node) => {
      ...
      if (node.isBone || node.type === "Bone") {
        ...
        const isLeg =
          (name.includes("leg") || name.includes("thigh")) &&
          !name.includes("lower") &&
          !name.includes("calf");
    ...
    if (foundMeshes.length > 0) {
      ...
      // Resolve emotion morph target indices from dictionary
      const EMOTION_KEYS = [
        ["Neutral", "neutral"],
        ["Happy", "happy", "Joy", "Smile", "Fcl_ALL_Fun"],
        ["Thinking", "thinking", "Surprise", "Fcl_ALL_Surprise"],
        ["Serious", "serious", "Angry", "Fcl_ALL_Angry"],
        ["Sad", "sad", "Sorrow", "Fcl_ALL_Sorrow"],
        ["Confident", "confident", "Determined"],
      ];
      emotionIndicesRef.current = EMOTION_KEYS.map((candidates) => getIdx(candidates));
      ...
      vowelsRef.current = newVowels.length > 0 ? newVowels : [];
      if (newVowels.length === 0) {
        console.warn("MyAvatar: No vowel morph targets found — lip-sync will be disabled.");
      }
    }
  }, [nodes]);
  ...
  useFrame((state) => {
    ...
    // Build target weights for emotion morph targets
    const emotionTargets = emotionIndicesRef.current;
    ...
    meshesRef.current.forEach((mesh) => {
      ...
      // Apply emotion morph targets via resolved indices
      emotionTargets.forEach((idx, i) => {
        if (idx !== -1 && idx < mesh.morphTargetInfluences.length) {
          const target = i === externalEmotion ? 1 : 0;
          mesh.morphTargetInfluences[idx] = THREE.MathUtils.lerp(mesh.morphTargetInfluences[idx], target, 0.1);
        }
      });
