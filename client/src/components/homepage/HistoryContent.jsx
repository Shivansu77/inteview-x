...
import { FiTrash2, FiChevronDown, FiChevronUp, FiCheck, FiX, FiAward } from "react-icons/fi";
...
const HistoryContent = ({ sessions, onDeleteSession }) => {
      ...
      {sessions.length === 0 ? (
        ...
        <div className="history-sessions">
          {sessions.map((session, sIdx) => (
            <div key={`${session.date}-${sIdx}`} className="history-session-card">
              <div
              ...
              >
                <div className="session-header-left">
                  ...
                  <div className="session-meta">
                    ...
                    <p>{session.experience} · {formatDate(session.date)} · {session.questionHistory?.length || 0} questions</p>
                  </div>
                </div>
                ...
                <div className="session-body">
                  <div className="session-scores">
                    {[
                      ...
                      <div key={scoreData.label} className="session-score-pill">
                        <span className="pill-dot" style={{ background: scoreData.color }}></span>
                        <span className="pill-label">{scoreData.label}</span>
                        <span className="pill-value" style={{ color: scoreData.color }}>{scoreData.value ?? "—"}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="qa-history">
                    ...
                    {(session.questionHistory || []).map((qa, qIdx) => {
                      ...
                      return (
                        <div key={qKey} className="qa-card">
                          <div
                            className="qa-header"
